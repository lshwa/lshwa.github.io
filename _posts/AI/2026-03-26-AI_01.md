...
> - **`A*` : 한 명이 최단경로 찾기**
> - **Minimax / Expectimax : 상대가 있거나, 불확실성이 있는 게임**

현실의 많은 문제는 행동 하나 하나를 고르고, 세상이 확률적으로 반응

- **연속적인 의사결정 문제**
  - ex) Atari 게임 :
    - 화면을 보고 (Observation)
    - 조이스틱으로 행동 (action)
    - 점수나 보상(reward)를 받는 과정을 반복 

- **non-deterministic search**
  - 행동 -> 결과 (X)
  - 행동 -> 여러 가능 결과들 -> 그 확률 

> 행동의 가치는 **단일 결과**가 아니라 **기댓값(expected value)**로 계산되야 함. 



**Markov Decision Process (MDP)**

- **States `S`** : 가능한 상태들의 집합

- **Actions `A`** : 각 상태에서 할 수 있는 행동

- **Transtion Function `T(s, a, s')`** : 
...
- **Reward Function `R(s, a, s')`** :
  ...
  - s에서 a를 하고 s'로 갔을 때 받는 즉시 보상
  - **living penalty** : 오래 있는 걸 싫어한다. 
    - **Policy가 reward 설계에 따라 변한다.**

- **Start State** : 시작 상태

- **Discount Factor** : 미래 보상 가치를 얼마나 줄여서 볼지 정하는 값

- **Horizon** : 얼마나 긴 시간까지 볼지 



**MDP = 상대가 있는 minimax가 아닌 환경 chance node가 계속 끼어드는 expectimax 형 구조**



> **Markov란 무엇인가?**

- **현재 상태를 알면 과거 전체를 굳이 알 필요 없이 미래를 예측할 수 있다.**
  $$
  P(s_{t+1}\mid s_t) = P(s_{t+1}\mid s_t, s_{t-1}, s_{t-2}, \dots)
  $$

  - 현재를 알고 있으면, 과거는 더 이상 추가 정보가 아니다. 

- MDP는 상태를 기반으로 정책을 정의
  $$
  \pi : S \to A
  $$

  - 가능하기 위해 상태가 진짜로 **필요한 정보를 다 담고 있어야 함**



이제 원하는 것은 plan이 아니라 policy 

- Deterministic single-agent search : 
  - `start -> goal` 까지 가는 action sequence
  - 즉 **plan을 원했다.**

- **MDP에서**
  - 환경이 확률적이라서 미리 짜 둔 action sequence가 깨질 수 있다. 
  - Plan보다 **Policy가 더 필요**하다. 
...
- **각 상태마다 어떤 행동을 할지 정해둔 규칙**
- 왜 Policy가 더 적절한가?
  - 실제 실행 중에 어느 상태로 가게 될지 확률적으로 달라질 수 있다.
  - 그 상태에 도착했을 때 뭘 할지가 준비되어 있어야 한다. 



**Rewards**

- MDP에서는 보상이 하나가 아니라, **시간에 따라 계속 보상을 받는다.**
- 어떤 reward sequence가 좋을까?
  - **총합**
  - **지금 받는게 좋은가?**

- **Discounting : 미래는 덜 중요하게 본다.**

  - 미래의 보상은 현재보다 덜 가치 있다. 
  ...
  - WHY?
    - **수학적으로 안정됨** : 무한 합이 수렴
    - **Infinite reward 문제 해결**



> 게임이 끝나지 않는다면? **(Infinite Horizon)**

- 둘 다 무한인데 뭐가 더 좋은지 비교가 불가능 

- 해결 방법 2가지

  - **Finite Horizon** : 일정 step 지나면 끝내기
...
- **V-value (state value) : `V*(s)`**
  ...
  - 상태 s에서 시작해서 최적으로 행동했을 때 얻는 기대 reward

- **Q-value (action value) : `Q*(s,a)`**

  - s에서 a를 먼저 하고 그 이후 최적으로 행동했을 때의 기대 reward 
...
2. **행동의 가치는 다음 상태들의 기대값, reward + 다음 상태 가치**
...
> **MDP = Expectimax + 반복 + Infinite Horizon**
>
> - state -> action 선택 (max) -> 확률 전이 (expectation) -> 다음 state ... 반복
...
> 해결 방법 : depth를 제한해서 계산하고, 점점 깊이를 늘리자.
...
- **k step까지만 봤을 때 최적 value**
- Depth-k exepctimax 결과 
  - k = 0 : 전부 0
  - k = 1 : reward  있는 곳만 값 생김
  - k = 2 : 주변으로 퍼짐 
  - ....
- 멀리 있는 보상도 점점 현재 상태에 영향을 준다. 
...
> 현재 상태의 가치 = **(최적 행동을 했을 때) 즉시 reward + 미래 가치의 기대값**

**Value Iteration = 1-step expectimax 반복**



- **Value Iteration을 계속 반복하면 결국 최적 가치함수에 도달한다.**
  $$
  V_k(s) \leftarrow \max_a \sum_{s'} T(s,a,s')\big(R(s,a,s')+\gamma V_{k-1}(s')\big)
  $$

  - 현재 상태 s에서 가능한 행동 a를 모두 보고
  - 그 행동을 했을 때 갈 수 있는 다음 상태 s들을 확률적으로 평균내고 
  - 즉시 보상 R + 미래 가치를 합쳐서 그 중 **가장 좋은 행동을 택함.**

- **왜 수렴하는가?**

  - 계속 반복할 수록 먼 미래까지 반영 가능
  - **discount factor가 1보다 작으면** 미래의 영향은 점점 작아진다. 
...
- **어떤 상태의 최적 가치**
  - **그 상태에서 할 수 있는 행동들 중 가장 좋은 행동을 골랐을 때의 기대 보상**



- 최적가치를 구했기에 **Optimal Policy**도 추출 가능 

$$
\pi^*(s)=\arg\max_a \sum_{s'} T(s,a,s')\big(R(s,a,s')+\gamma V^*(s')\big)
$$

- **Optimal Action = Highest 행동의 가치를 주는 행동**
  $$
...
> max : 값 자체를 구함
>
> argmax : 그 값을 만들어내는 행동을 구함. 



**Value Iteration Property**

- 한 번의 iteration마다 모든 state를 돌고, 각 state마다 모든 action과 다음  state를 본다. 
  $$
  O(|S| \cdot |A| \cdot |\text{next states}|)
  $$
...
- optimal policy를 구하는 **대안적 접근**
  - optimal value를 계속 갱신 X
  - **policy를 직접 업데이트**

**Step 1. Policy Evaluation**

- 그 정책을 계속 따를 때 각 상태의 가치 계산

**Step 2. Policy Improvement**

- 각 상태에서 한 step lookahead를 보고 
- 현재 정책보다 더 좋은 행동이 있으면 그걸로 바꿈 
...
- 최적으로 행동하는게 아니라 **정책을 고정해서** 그 정책을 따라갈 때의 기대 효용 
...
**1. random policy** : 아무 정책이나 하나 정함

**2. Policy Evaluation** : 그 정책의 ` V^\pi` 계산

**3. Policy Improvement** : 그 ` V^\pi`를 이용해서 더 나은 Policy 생성

**4. Iterative** : Policy가 더 이상 안 바뀔 때까지 반복 



> 요약

**1. Optimal Policy인 경우**
...
- **Iterative Updates**
  - Policy가 고정된 상태로 반복 계산 
- **Linear System으로 풀기**
  - 식에 max가 없으니 **연립일차방정식**으로 푼다. 
